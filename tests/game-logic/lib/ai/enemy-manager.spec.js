...
// ---------------------------------------------------------------------------
// Tests merged from enemy-manager-core (formerly enemy-manager.test.js)
// ---------------------------------------------------------------------------

// Additional EnemyManager exports used by merged tests
const {
...
// Additional tile factories used by merged tests (not already defined above)
function makeTile(row, col, sprite, overlay) {
    const t = { row, col, sprite };
    if (overlay) t.overlay = overlay;
    return t;
}
const gate   = (r, c) => makeTile(r, c, 'castle-gatehouse');
const keepTL = (r, c) => makeTile(r, c, 'castle-keep-tl');
const keepBL = (r, c) => makeTile(r, c, 'castle-keep-bl');
const keepBR = (r, c) => makeTile(r, c, 'castle-keep-br');
const oak    = (r, c) => makeTile(r, c, 'grass-short-1', 'tree-oak-overlay-1');
