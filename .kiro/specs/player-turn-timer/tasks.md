...
- `unitStepIntervalMs = 1_000` (1 second per unit move) is the v1 baseline. It gives the player enough time to visually track each unit without the enemy phase feeling sluggish. Highlighting the currently moving unit and adding a recognisable movement sound cue are explicitly out of scope for v1 — they will be addressed in a future HUD enhancement spec.
