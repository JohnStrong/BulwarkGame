...
- **LastSeenRegistry**: A `Map<stableUnitKey, { row, col, turn }>` maintained by EnemyManager. Records the last physically observed position and observation turn of every player unit that any enemy unit has ever sighted. Each entry is keyed by a stable unit identity key that does not change when the unit moves — ensuring that re-sighting a moved unit updates the existing entry rather than creating a duplicate. Entries persist until a new sighting updates them, the unit is killed, or the entry expires after `SIGHTING_EXPIRY_TURNS` turns of no re-sighting. Cleared on level restart.
...
1. THE EnemyManager SHALL maintain a `LastSeenRegistry` — a `Map<playerId, { row, col, turn }>` — storing the last observed position and observation turn for every player unit that has ever been sighted by any enemy unit. Each entry is keyed by a **stable unit identity key** that remains constant regardless of the unit's current position (see Requirement 13.15).
2. WHEN any active EnemyUnit's `computeEnemyVisibleTiles` set contains a tile occupied by a player unit (cross-referenced against `UnitManager.getPlacedUnits()`), THE EnemyManager SHALL immediately **update** (upsert) the `LastSeenRegistry` entry for that player unit with the current position and current turn number. This SHALL replace the previous entry for the same unit — the registry SHALL NOT create a new entry for the same unit at the new position.
...
7. WHEN a player unit is killed, THE game's Resolve Phase SHALL call `EnemyManager.notifyUnitKilled(killedUnit)` immediately at the point of death — passing the killed player unit object. `notifyUnitKilled` SHALL derive the stable key from the unit via `_stableKeyFor` and immediately remove that unit's entry from the `LastSeenRegistry` and its cache entry from `_unitKeyCache`. This event-driven removal means the enemy force stops routing around the killed unit's position within the same turn's next pathfinding call.
7a. AS A SAFETY NET: at the start of each `executeTurn()` call, THE EnemyManager SHALL also run a fallback sweep (`_safetyPurgeDead`) that cross-references all current registry keys against `UnitManager.getPlacedUnits()` and removes any entries whose unit is no longer present. This guards against missed `notifyUnitKilled` calls during the period before full combat logic is implemented. Once the Resolve Phase reliably calls `notifyUnitKilled` for every death, the fallback sweep has no work to do and imposes negligible overhead.
...
15. THE stable identity key for a player unit in the `LastSeenRegistry` SHALL be derived once from the unit's intrinsic identity, not from its current position. The key SHALL be `unit.id` if the player unit object exposes a stable `id` property, otherwise `unit.name + ':' + unit.spawnIndex` or any other stable composite derived from unit definition fields that does not change when the unit moves. A position-based fallback (`tileKey(row, col)`) SHALL NOT be used as the registry key, because it changes when the unit moves and would cause the registry to accumulate duplicate entries — one for the old position and one for the new. IF no stable identity field is available on the player unit object, THE EnemyManager SHALL derive a key from `UnitManager.getPlacedUnits()` index position at the time of the unit's first sighting and cache it for the lifetime of that unit's presence on the map.
