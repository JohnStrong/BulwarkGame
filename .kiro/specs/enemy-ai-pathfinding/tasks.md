...
- [ ] 8. Implement `EnemyManager.executeTurn()` — pathfinding and movement
  - [ ] 8.1 Implement the `executeTurn()` movement loop
    ...
    - Call `PathfindingEngine.buildSharedThreatMap(UnitManager.getPlacedUnits(), this._units, tileGraph)` ONCE per turn; store as `_sharedThreatMap`
...
- [ ] 12. Implement `PathfindingEngine.hexRing`, `computeEnemyVisibleTiles`, and `buildSharedThreatMap`
  ...
  - [ ] 12.3 Implement `PathfindingEngine.buildSharedThreatMap(placedUnits, activeEnemyUnits, tileGraph)`
    - Build collective enemy visible set: union of `computeEnemyVisibleTiles` for all `activeEnemyUnits`
    - For each player unit: set `overlay.set(tileKey(unit.row, unit.col), 3)` (combat cost — always applied)
    ...
    - Never lower an existing overlay entry; combat tile stays at 3 even inside a threat zone
    - Return empty Map when both `placedUnits` and `activeEnemyUnits` are empty/null
    - Expose on `PathfindingEngine`
    - _Requirements: 9.1, 9.2, 9.3, 9.4, 9.5, 9.7, 10.2, 11.5, 11.6, 11.9_
...
- `buildSharedThreatMap` signature updated: now takes `(placedUnits, activeEnemyUnits, tileGraph)` — update all call sites and test stubs.
...
```json
{
  "waves": [
    ...
    { "id": 4, "tasks": ["6.5", "7.2", "12.1", "12.2"] },
    { "id": 5, "tasks": ["8.1", "12.3"] },
    { "id": 6, "tasks": ["8.2", "10.1", "12.4", "12.5"] },
