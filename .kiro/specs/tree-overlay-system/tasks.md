...
- [x] 2. Register overlay sprites in SpriteManager
  ...
  - [x] 2.2 Write unit tests for SpriteManager overlay registration
...
- [x] 5. Update level loader for O/P/S tile objects
  - [x] 5.1 Update the `O`, `P`, and `S` switch cases in `LevelLoader.parseLevelText` in `js/game-logic/level-loader.js`
  ...
  - [x] 5.4 Write unit tests for level loader tree tile changes
...
- [x] 6. Implement two-pass rendering in IsoRenderer
  - [x] 6.1 Add `TREE_OVERLAY_OFFSET_Y`, `OVERLAY_WIDTH`, and `OVERLAY_HEIGHT` constants at the top of `js/game-logic/lib/iso-renderer.js`
    - `TREE_OVERLAY_OFFSET_Y = 0`, `OVERLAY_WIDTH = 64`, `OVERLAY_HEIGHT = 48`
    - _Requirements: 5.1_

  - [x] 6.2 Update `IsoRenderer.drawTerrain` to perform two-pass rendering
  ...
  - [x] 6.6 Write unit tests for IsoRenderer two-pass rendering
...
- [x] 8. Integrate overlay sprites into the build pipeline
  - [x] 8.1 Add the overlay generator script to `GENERATOR_SCRIPTS` in `js/level-generators/build-sprites.js`
  ...
  - [x] 8.2 Write unit tests for build pipeline overlay integration
